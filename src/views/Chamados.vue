...
export default {
    ...
    created(){
        axios.get("http://localhost:8989/chamados").then(res =>{
    ...
    methods: {
        ...
        showChamadoUSer(chamado_id){
              axios.get("http://localhost:8989/chamado/"+localStorage.getItem('id')+"/"+chamado_id).then(res =>{
        ...
        deletarChamado(){
            axios.delete("http://localhost:8989/chamado/"+localStorage.getItem('id')+"/"+this.chamadoId).then(res =>{
                this.showModal = false
                this.$router.go()
                return res
            }).catch(erro =>{
        ...
        statusChamado(chamado_id){
          axios.patch("http://localhost:8989/chamado/"+localStorage.getItem('id')+"/"+chamado_id+"/status").then(res =>{
            this.$router.go() 
              return res;
            }).catch(erro => {
              console.log(erro.response)
